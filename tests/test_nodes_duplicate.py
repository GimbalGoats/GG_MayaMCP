...
def test_nodes_duplicate_success(mock_client):
    ...
    # Mock Maya response
    mock_response = {"duplicated": {"pCube1": "pCube2"}, "errors": {}}
...
def test_nodes_duplicate_single_with_name(mock_client):
    ...
    # Mock Maya response
    mock_response = {"duplicated": {"pCube1": "myCube"}, "errors": {}}
    mock_client.execute.return_value = json.dumps(mock_response)
...
def test_nodes_duplicate_options(mock_client):
    ...
    nodes_duplicate(nodes=["pCube1"], input_connections=True, upstream_nodes=True, parent_only=True)
