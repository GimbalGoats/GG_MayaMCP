...
- Maya 2024 commandPort setup now uses `echoOutput=False`. With `echoOutput=True`, Autodesk's `CommandPort.py` crashes on Python 3 at the top of every connection while flushing its command-output queue (it writes `str` to a binary socket), *before* running the received command. That queue is shared across connections and is re-poisoned by the crash traceback itself, so it can never be drained from outside — every command, including the compatibility bootstrap, was silently swallowed and Maya reported "commandPort accepts connections but is not executing commands." Opening the port with `echoOutput=False` bypasses the broken path entirely; the compatibility server then captures command output itself.
- The compatibility server now retries its port bind on Windows `WSAEACCES` (10013), not just `WSAEADDRINUSE` (10048). Right after it closes Maya's built-in commandPort, Windows briefly refuses the rebind with 10013 during socket teardown; the missing retry made the takeover fail and leave Maya without any listener.
...
- Safely embedded JSON values in `attributes.set` and `nodes.create` Maya-side
